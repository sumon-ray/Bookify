...
export default function Page() {
    ...
    if (takeBooks?.length === 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.post(`https://bookify-server-lilac.vercel.app/take-book?email=${session?.user?.email}&AuthorEmail=${book?.AuthorEmail}&id=${book?._id}`,
            { ...book, requester: session?.user?.email, bookId: book?._id, })
            .then(res => {
              if (res.data.message) {
                Swal.fire({
  ...
  async function giveBooksExchange(book) {
    try {
      const response = await axios.post(`https://bookify-server-lilac.vercel.app/give-book?id=${book?._id}`,
        {
          ...book,
          requester: user,
          bookId: book?._id,
      ...
      toast.success(response.data.message);
      giveBooksRefetch(); // Refetch to get updated data
    } catch (error) {
  ...
  if (status === "loading") {
    return <div className="flex justify-center items-center w-full min-h-[70vh] pt-20"><svg
        ...
        <div className="lg:w-[50%] md:h-[500px] w-full bg-[#EFEEE9] rounded-lg md:p-6">
          ...
          <p className="font-semibold text-center text-[#000000]">Add the books you are giving in exchange here</p>
          {
            takeBooksLoading ?
              ...
              :
              <div className="grid lg:grid-cols-3 grid-cols-2 gap-x-8 gap-y-6 px-6 h-[360px] mt-6 overflow-y-auto">
...

                {
                  takeBooks?.map(takeBook => (
                    <div key={takeBook?._id} className="relative h-40">
                      ...
                      />
                      <button>
                        <MdOutlineDeleteOutline onClick={() => giveBookDelete(takeBook?._id)}
