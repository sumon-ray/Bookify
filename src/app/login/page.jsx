...
const DEMO_EMAIL = "user1@gmail.com"; 
const DEMO_PASSWORD = "40114345"; 
...
const LoginForm = ({ redirect }) => {
    ...
    const handleLogin = async (e) => {
        ...
        const resp = await signIn("credentials", {
            ...
            password,
            redirect: false,
            // callbackUrl: redirect || "/", // এটা এখন দরকার নেই কারণ আমরা ম্যানুয়ালি রিডাইরেক্ট করব
        });
    ...
    const handleDemoLogin = async () => {
        ...
        if (resp?.ok) {
            ...
            setError(resp?.error || "Demo login failed. Please check demo credentials.");
            toast.error(resp?.error || "Demo login failed.");
        }
    };
    ...
    return (
        <form onSubmit={handleLogin}>
            ...
            {error && <div className="text-red-500 mt-2 text-sm">{error}</div>} {/* Add margin top */}
                ...
                >
                    {loading ? (
                        <div className="flex items-center text-sm font-medium justify-center">
                            <svg
                                className="animate-spin border-[#364957] h-5 w-5" // Added h-5 w-5 for better size
            ...
            {/* ডেমো লগইন বাটন যোগ করা হয়েছে */}
            <div className="mt-4">
                <button
                    type="button" // Type should be "button" to prevent form submission
                    onClick={handleDemoLogin}
                    className="w-full px-6 py-3 text-sm font-medium tracking-wide bg-blue-600 text-white capitalize transition-colors duration-300 transform rounded-lg hover:bg-blue-700 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-50"
                    disabled={loading}
                >
...
const Page = ({ redirect }) => {
    // আপনার `img` এবং `img2` import গুলো ঠিক করে নিন
    // import img from "../../../src/assets/images/About/logo (1).png"; // এটি সঠিক পাথ নাও হতে পারে
    // import img2 from "../../../src/assets/images/About/bookdark.png"; // এটি সঠিক পাথ নাও হতে পারে
    // যদি আপনি ইমেজগুলো পাবলিক ফোল্ডারে রাখেন, তাহলে শুধু '/images/logo.png' লিখলেই হবে।
    // অথবা, যদি src ফোল্ডারের মধ্যে থাকে, তাহলে সঠিক রিলেটিভ পাথ দিন।
        ...
        <Suspense fallback={<div>Loading...</div>}>
            <div className="flex justify-center w-full h-screen mx-auto overflow-hidden rounded-lg dark:bg-[#272727A6] dark:text-white">
                ...
                <div className="w-full mx-6 lg:px-16 md:px-8 lg:w-1/2 flex flex-col justify-center my-4">
                    <Link href={"/"}>
                        <div className="mt-4 flex items-center justify-center text-[#B7B7B7]">
                            <div className="dark:hidden">
                                <Image
                                    src="https://i.ibb.co/M9C3ZJ0/logo.png" // আপনার লোগো ইমেজের সঠিক পাথ দিন
                            ...
                            <div className="hidden dark:block">
                                <Image
                                    src="https://i.ibb.co/C0W2V9j/bookdark.png" // আপনার ডার্ক মোড লোগো ইমেজের সঠিক পাথ দিন
                                    className="h-14 md:h-[68px] w-28 md:w-36"
                                    height={1100}
                                    width={600}
                                    alt="Logo"
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <span className="w-1/5 border-b dark:border-gray-700 lg:w-1/4"></span> {/* Changed border color for dark mode consistency */}
                        <p className="text-xs text-center text-gray-500 uppercase dark:text-white">
                            or login with email
                        </p>
                        <span className="w-1/5 border-b dark:border-gray-700 lg:w-1/4"></span>
                    </div>
                    <LoginForm redirect={redirect} />{" "}
                    {/* Pass the redirect parameter here */}
                    <div className="flex items-center justify-between my-4">
                        <span className="border-b dark:border-gray-700 md:w-1/4"></span>
                        <span>
