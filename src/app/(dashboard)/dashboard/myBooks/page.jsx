...
export default function MyBooks() {
    ...
    const { data } = useQuery({
        queryKey: ['my books'],
        queryFn: async () => {
            const res = await axios(`https://bookify-server-lilac.vercel.app/books?email=${'abcd@gmail.com'}`)
        ...
        <section className="space-y-8 pb-8 md:pb-12 px-5 md:px-0">
            ...
            <div className="bg-[#EFEEE9] rounded-md p-5 md:p-2 flex flex-col-reverse md:flex-row items-center justify-between px-4 md:px-20">
                ...
                <div className="space-y-3 pt-5 md:pt-0">
                    <h3 className="text-xl md:text-5xl font-bold">Falling in love <br />one page at a time</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
                ...
                {/* card */}
                {data?.map(book =>
                    <div key={book?._id} className="w-56 md:w-auto bg-[#ffffff] border border-[#00000099] flex flex-col justify-center items-center rounded-lg p-5">

                        <div className="space-y-3">
                            <Image src={book?.coverImage} className="w-44 md:w-56 h-48 md:h-52 rounded-lg" height={150} width={200} alt="" />
