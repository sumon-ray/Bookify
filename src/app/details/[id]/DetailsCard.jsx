...
export default function DetailsCard({ genre, title }) {
  ...
  const { data: relatedBook, isLoading, isFetching, isPending } = useQuery({
    queryKey: ['details card', genre, title],
    queryFn: async () => {
      const res = await axios(`https://bookify-server-lilac.vercel.app/books?genre=${genre}&title=${title}`)
