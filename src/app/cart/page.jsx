...
const Page = () => {
  ...
  const { data } = useQuery({
    queryKey: ['cart'],
    queryFn: async () => {
      const res = await axios(`https://bookify-server-lilac.vercel.app/cart?email=${session?.user?.email}`)
  ...
  return (
   <div className="pt-20">
    ...
    <div className="bg-[#EFEEE9] p-6 text-black ">
      <h1 className="text-3xl font-black uppercase text-center dark:text-white">My Cart</h1>
    </div>

     <div className="md:mt-20 mb-20 max-w-7xl mx-auto">
    
       {/* table */}
    
       <div className="relative overflow-x-auto shadow-m sm:rounded-g">
    
         <table className="w-full text-sm text-left rtl:text-right rounded-md">
    
           <thead className="text-xs bg-white rounded-t-xl text-black uppercase">
             <tr>
           ...
           <tbody>
             {data?.map((book, i) => (
               <tr
                 key={i}
                 className="border-b border-gray-300 hover:bg-[#F3F2ED]">
    
                 <td className="p-4">
                   <img src={book?.coverImage} className="w-16 md:w-32 max-w-full max-h-full object-fill rounded-md" alt={book?.Title} />
                 </td>
       ...
       <div className="w-full flex flex-col lg:flex-row gap-8 md:justify-between justify-center items-center">
