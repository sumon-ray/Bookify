...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://bookify-server-lilac.vercel.app/book/${id}`);
        const data = await response.json();

        if (response.ok) {
          const cleanedContent = data.content.replace(/\\n/g, ' ').replace(/\s{2,}/g, ' ').trim();
          const parsedContent = cleanedContent.startsWith('[') && cleanedContent.endsWith(']')
  ...
  // Navigation handlers
  const handleNext = () => setCurrentPage((prev) => Math.min(prev + 2, book.content.length - 2));
